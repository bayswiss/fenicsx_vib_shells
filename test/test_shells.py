...
def _compute_modal_frequencies(
    ...
    """Load mesh, apply BCs, solve GHEP. Returns freqs [Hz]."""
    if comm is None:
        comm = MPI.COMM_WORLD
...
def _assert_modes(freqs, ref, tol, label):
    """Print table. Assert max rel err < tol."""
...
def test_plate_modes():
    ...
    f_factor = 1/(2*np.pi*0.3**2)*np.sqrt(D/(material.rho*material.h))
